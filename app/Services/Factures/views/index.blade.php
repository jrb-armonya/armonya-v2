                        ...
                        @foreach($factures as $facture)
                            <tr>
                                ...
                                @if($facture->status == "vide")
                                    <td class="badge badge-status" style="color: white; background-color: #06c8c3;">
                                        {{$facture->status}}
                                    </td>
                                @elseif($facture->status == "En Attente")
                                    <td class="badge badge-status" style="color: white; background-color: #9b9b00;">
                                        {{$facture->status}}
                                    </td>
                                @elseif($facture->status == "Envoyée")
                                    <td class="badge badge-status" style="color: white; background-color: #ef9c03;">
                                        {{$facture->status}}
                                    </td>
                                @endif
                                <td>{{ $facture->partenaire->fichesCibled()->where('facture_id', null)->count() }}</td>
                                ...
                                <td>
                                    ...
                                    @else
                                    <a href="{{ url( '/facturation/factures/preview', $facture->id ) }}"><i class="ti-search btn-open-fiche"></i></a>
                                    @endif
